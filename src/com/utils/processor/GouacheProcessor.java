 ...
 * The processor transforms an image into a gouache painting style.
 * @author William
 *
 */
public class GouacheProcessor implements Processor {

	
	private volatile static GouacheProcessor uniqueInstance = null;
	private int radius = 5;
	private int intensity = 7;
	
	
	private GouacheProcessor(){
	}
	
	/**
	 * the public method to get the unique instance
	 * @return GouacheProcessor the unique instance of this class
	 */
	public static GouacheProcessor getInstance(){
        if(uniqueInstance == null){
            synchronized(GouacheProcessor.class) {
                if(uniqueInstance == null) {
                    uniqueInstance = new GouacheProcessor();
                }
            }
        }
